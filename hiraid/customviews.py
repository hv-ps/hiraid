...
class Customviews(Views):
    ...
    def gethostgrpdetail_hostgroupsbyportname(self,metaview,viewname):
        view = {}
        for port in metaview['data']:
            for gid in metaview['data'][port]:
                if port not in view:
    ...
    def getlun_lunsbyportgid(self,metaview,viewname):
        view = {}
        self.log.debug("view_keyname: "+viewname)
        for port in metaview['data']:
            for gid in metaview['data'][port]:
                port_gid = port+"-"+gid
                view[port_gid] = metaview['data'][port][gid]
    ...
    def getldev_forcevirtualldevid(self,metaview,viewname):
        view = {}
        self.log.debug("view_keyname: "+viewname)
        for ldevid in metaview['data']:
            if 'VIR_LDEV' not in metaview['data'][ldevid]:
                metaview['data'][ldevid]['VIR_LDEV'] = ldevid
                virtualculdev = self.storage.returnldevid(ldevid)['culdev']
                metaview['data'][ldevid]['CULDEV_VIR_LDEV'] = virtualculdev
