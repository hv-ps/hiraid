            ...
            for group in pairdisplay_dict['pairs']:
                for pairvol in pairdisplay_dict['pairs'][group]:                    
                    ...
                    try:
                        ...
                        pairdisplay_dict['pairs'][group][pairvol]['L']['GB'] = round(storagecaps(int(volume_capacities[serial][ldev_id]),'blks').GB,2)
                        pairdisplay_dict['pairs'][group][pairvol]['L']['REPGB'] = round(storagecaps((int(volume_capacities[serial][ldev_id])/100)*int(percent),'blks').GB,2)
                        pairdisplay_dict['totalgb'] += round(storagecaps(int(volume_capacities[serial][ldev_id]),'blks').GB,2)
                        pairdisplay_dict['totalrepgb'] += round(storagecaps((int(volume_capacities[serial][ldev_id])/100)*int(percent),'blks').GB,2)
                    except Exception as e:
